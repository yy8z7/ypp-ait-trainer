 ...
 */
import { state, loadState, saveState, resetState, getTodayString, config } from './store.js';
import {
    dom,
    renderHome,
    switchScreen,
...
function startSession() {
    if (state.currentDay > config.totalDays) return;
...
function resumeSession() {
    ...
    // Fix backwards compatibility: purge any long cooldowns generated by old logic
    // so user is not stuck upon resuming a previously locked session.
    let now = Date.now();
    sessionQueue.forEach(q => {
        if (q.showAfter > now) {
            q.showAfter = 0;
        }
    });
...
function processNextQuestion() {
    ...
    if (idx === -1) {
        let minWait = Math.min(...sessionQueue.map(q => q.showAfter)) - now;
        let waitMins = Math.ceil(minWait / 60000);
        waitMins = waitMins < 1 ? 1 : waitMins;
        alert(`剩下的题目还在记忆冷却阶段，为保证抗遗忘效果，请休息约 ${waitMins} 分钟后再来继续！`);
        saveSessionState();
        switchScreen(dom.homeScreen);
        renderHome();
        return;
...
function renderCard(qState) {
    ...
    if (!qData) {
        console.warn('Question missing in database, auto skipping:', qState.id);
        markQuestionCompleted(qState.id);
        currentQ = null;
        saveSessionState();
...
function checkAnswer(qData, optionsElements) {
    if (!currentQ) return; // Prevent double execution
    let selected = [];
    ...
    let isCorrect = false;
    let normalizedCorrectAnsArr = [];
    
    if (qData.type === 'multiple') {
        normalizedCorrectAnsArr = correctAns.match(/[A-Z]/g) || [correctAns];
        let normalizedCorrectAnsStr = normalizedCorrectAnsArr.sort().join(',');
        isCorrect = (userAns === normalizedCorrectAnsStr);
    } else {
    ...
    if (isCorrect) {
        ...
        // 当日无论是否原为错题，二次答对直接出队，不再卡 3 次与 60 分钟冷却
        ...
        currentQ.step = 1;
        // 降低冷却，0 意味着推入队尾（若有其他题先做其他题）
        currentQ.showAfter = 0;
        ...
        optionsElements.forEach((opt) => {
            ...
            if (qData.type === 'multiple') {
                if (normalizedCorrectAnsArr.includes(opt.dataset.val)) opt.classList.add('correct');
            } else {
        ...
        document.getElementById('next-q-btn').addEventListener('click', () => {
            showToast('已加入今日重新队列，再次答对后进度才会+1哦~');
            nextQuestion();
        });
    }
}
...
function finishRound() {
    ...
    if (isUltimateChallenge) {
        ...
        if (rate >= 0.9) {
            state.currentDay = config.totalDays + 1;
            saveState();
            switchScreen(dom.homeScreen);
